...
### 소감
key<->value 쌍방으로 찾을 때 map을 두 개를 선언하면 된다는 인사이트를 얻었다.   
그리고,, c++에는 atoi를 통해 숫자, 문자를 구별하는데(return 0 or 1) 자바에서 Integer.parseInt(문자)를 하면 exception이 발생한다. 
exception 처리는 직관적이지 않으므로 차라리 아스키코드로 변환하여 판별하는게 나을 것 같다. 
